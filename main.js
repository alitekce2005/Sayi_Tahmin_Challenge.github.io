...
function endGame(won) {
    ...
    if (won) {
        ...
        // --- ÖZEL PUANLAMA SİSTEMİ ---
        const livesLeft = maxAttempts - currentAttempt + 1;
        let earnedScore = 0;
        let rankText = "";

        // Senin belirlediğin puanlar:
        if (livesLeft === 4) {
            earnedScore = 100;
            rankText = "(EFSANE!)";
        } else if (livesLeft === 3) {
            ...
            earnedScore = 10; // 1 Can
            rankText = "(Kılpayı)";
        }

        messageText.innerHTML = `Neon kodu ${currentAttempt}. denemede çözdünüz!<br>
        <div style="margin-top:15px;">
            <span style="color:var(--neon-green); font-size:1.4rem; font-weight:bold;">+${earnedScore} Bilgi Puanı</span><br>
            <span style="color:#fbbf24; font-size:0.9rem;">${rankText}</span>
        </div>`;
    ...
    if (navigator.vibrate) {
        if (won) navigator.vibrate([200, 100, 200, 100, 200]);
        else navigator.vibrate([500, 200, 500]);
    }
}
...
function sendScoreToParent(score) {
    // '*' sayesinde hem localhost hem github.io mesajı alabilir.
    window.parent.postMessage({
        type: 'GAME_OVER',
        gameId: 2,               // Sayı Tahmin ID'si
        score: score,            // Puan
        pointName: 'Bilgi Puanı' // Puan Adı
    }, '*');
